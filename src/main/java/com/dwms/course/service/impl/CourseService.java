...
public class CourseService implements ICourseService {
    ...
    public ResultBean getDetail(DetailForm form) {
        DetailVO d = courseDAO.selectDetail(form.getCourseId(), form.getUserId());
        if (d == null) {
    ...
    public ResultBean saveLearn(LearnForm form) {
        ...
        Course c = courseDAO.selectByPrimaryKey(form.getCourseId());
        if (c == null) {
            return ResultUtils.errorBean(StatusEnum.NOT_FOUND, "课程不存在");
        }
        ...
        if (cu == null) {
            ...
            cu.setTimeLength(c.getTimeLength());
            cu.setStatus(CourseConsts.USER_STATUS_GOING);
            cu.setProgress(0);
            cu.setRestChapter(c.getChapterNum());
