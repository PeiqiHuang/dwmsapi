//package com.dwms.common.component.swagger2;
//
...
//        Optional<ResolvedType> operationResponse = operationAnnotation.transform(resolvedTypeFromOperation(typeResolver, defaultResponse));
...
//
//                    responseMessages.add(new ResponseMessageBuilder().code(apiResponse.code()).message(apiResponse.message()).responseModel(responseModel.orElse(null)).headersWithDescription(headers).build());
//                }
//            }
//        }
//        if (operationResponse.isPresent()) {
//            ModelContext modelContext = returnValue(context.getGroupName(), operationResponse.get(), context.getDocumentationType(), context.getAlternateTypeProvider(), context.getGenericsNamingStrategy(), context.getIgnorableParameterTypes());
...
//            ResponseMessage defaultMessage = new ResponseMessageBuilder().code(httpStatusCode(context)).message(message(context)).responseModel(responseModel).build();
...
//
//
//    private java.util.Optional<ResolvedType> resolvedType(ResolvedType resolvedType, ApiResponse apiResponse) {
//        ResolvedType r = resolvedTypeFromResponse(typeResolver, resolvedType).apply(apiResponse);
//        return r == null ? java.util.Optional.empty() : java.util.Optional.ofNullable(r);
//
//        //        (nullableReference == null) ? Optional.<T>absent() : new Present<T>(nullableReference);
